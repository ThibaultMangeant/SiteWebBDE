...
class AuthController extends Controller {
	...
	public function register() {
		...
		if (!empty($postData))
		{
			...
			if($utilisateur === null)
			{
				$utilisateur = new Utilisateur(0, $this->getPostParam('numero_etudiant'),$this->getPostParam('password'),$this->getPostParam('nom'),$this->getPostParam('prenom'));
				if($utilisateurRepository->create($utilisateur))
				{
					$authService->setUtilisateur($utilisateur);
					$this->redirectTo('index.php');
				}
		...
		$this->view('register.html.twig', $data ); // Affiche la vue login.php
	}
}